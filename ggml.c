...
static void quantize_row_q4_0(const float * restrict x, void * restrict y, int k) {
    quantize_row_q4_0_reference(x, static_cast<block_q4_0 *>(y), k);
}
...
static void quantize_row_q4_1(const float * restrict x, void * restrict y, int k) {
    quantize_row_q4_1_reference(x, static_cast<block_q4_1 *>(y), k);
}
...
static void quantize_row_q5_0(const float * restrict x, void * restrict y, int k) {
    quantize_row_q5_0_reference(x, static_cast<block_q5_0 *>(y), k);
}
...
static void quantize_row_q5_1(const float * restrict x, void * restrict y, int k) {
    quantize_row_q5_1_reference(x, static_cast<block_q5_1 *>(y), k);
}
...
static void quantize_row_q8_0(const float * restrict x, void * restrict vy, int k) {
    ...
    block_q8_0 * restrict y = static_cast<block_q8_0 *>(vy);
...
static void quantize_row_q8_1(const float * restrict x, void * restrict vy, int k) {
    ...
    block_q8_1 * restrict y = static_cast<block_q8_1 *>(vy);
...
static void dequantize_row_q8_0(const void * restrict vx, float * restrict y, int k) {
    ...
    const block_q8_0 * restrict x = static_cast<const block_q8_0 *>(vx);
...
static void ggml_vec_dot_q4_0_q8_0(const int n, float * restrict s, const void * restrict vx, const void * restrict vy) {
    ...
    const block_q4_0 * restrict x = static_cast<const block_q4_0 *>(vx);
    const block_q8_0 * restrict y = static_cast<const block_q8_0 *>(vy);
...
static void ggml_vec_dot_q4_1_q8_1(const int n, float * restrict s, const void * restrict vx, const void * restrict vy) {
    ...
    const block_q4_1 * restrict x = static_cast<const block_q4_1 *>(vx);
    const block_q8_1 * restrict y = static_cast<const block_q8_1 *>(vy);
...
static void ggml_vec_dot_q5_0_q8_0(const int n, float * restrict s, const void * restrict vx, const void * restrict vy) {
    ...
    const block_q5_0 * restrict x = static_cast<const block_q5_0 *>(vx);
    const block_q8_0 * restrict y = static_cast<const block_q8_0 *>(vy);
...
static void ggml_vec_dot_q5_1_q8_1(const int n, float * restrict s, const void * restrict vx, const void * restrict vy) {
    ...
    const block_q5_1 * restrict x = static_cast<const block_q5_1 *>(vx);
    const block_q8_1 * restrict y = static_cast<const block_q8_1 *>(vy);
...
struct ggml_tensor * ggml_set_i32 (struct ggml_tensor * tensor, int32_t value) {
    ...
    char * const data = static_cast<char * const>(tensor->data);
...
static void ggml_compute_forward_mul_mat_f16_f32(
    ...
    if (params->type == GGML_TASK_INIT) {
        ggml_fp16_t * const wdata = static_cast<ggml_fp16_t *>(params->wdata);
    ...
    ggml_fp16_t * wdata = static_cast<ggml_fp16_t *>(params->wdata);
...
void ggml_graph_compute(struct ggml_context * ctx, struct ggml_cgraph * cgraph) {
    ...
    struct ggml_compute_state * workers = static_cast<ggml_compute_state *>(n_threads > 1 ? alloca(sizeof(struct ggml_compute_state)*(n_threads - 1)) : NULL);
...
static enum ggml_opt_result ggml_opt_adam(
    ...
    float * x  = static_cast<float *>(ggml_new_tensor_1d(ctx, GGML_TYPE_F32, nx)->data); // view of the parameters
    float * g1 = static_cast<float *>(ggml_new_tensor_1d(ctx, GGML_TYPE_F32, nx)->data); // gradient
    float * g2 = static_cast<float *>(ggml_new_tensor_1d(ctx, GGML_TYPE_F32, nx)->data); // gradient squared
    float * m  = static_cast<float *>(ggml_new_tensor_1d(ctx, GGML_TYPE_F32, nx)->data); // first moment
    float * v  = static_cast<float *>(ggml_new_tensor_1d(ctx, GGML_TYPE_F32, nx)->data); // second moment
    float * mh = static_cast<float *>(ggml_new_tensor_1d(ctx, GGML_TYPE_F32, nx)->data); // first moment hat
    float * vh = static_cast<float *>(ggml_new_tensor_1d(ctx, GGML_TYPE_F32, nx)->data); // second moment hat

    float * pf = static_cast<float *>(params.past > 0 ? ggml_new_tensor_1d(ctx, GGML_TYPE_F32, params.past)->data : NULL); // past function values
    ...
    while (true) {
        ...
        if (*fx > finit + (*step)*dgtest) {
            ...
            if (params->lbfgs.linesearch == GGML_LINESEARCH_BACKTRACKING_ARMIJO) {
                return static_cast<ggml_opt_result>(count);
            }
...
static enum ggml_opt_result ggml_opt_lbfgs(
    ...
    float * x  = static_cast<float *>(ggml_new_tensor_1d(ctx, GGML_TYPE_F32, nx)->data); // current parameters
    float * xp = static_cast<float *>(ggml_new_tensor_1d(ctx, GGML_TYPE_F32, nx)->data); // previous parameters
    float * g  = static_cast<float *>(ggml_new_tensor_1d(ctx, GGML_TYPE_F32, nx)->data); // current gradient
    float * gp = static_cast<float *>(ggml_new_tensor_1d(ctx, GGML_TYPE_F32, nx)->data); // previous gradient
    float * d  = static_cast<float *>(ggml_new_tensor_1d(ctx, GGML_TYPE_F32, nx)->data); // search direction
    ...
    // the L-BFGS memory
    struct ggml_lbfgs_iteration_data * lm = static_cast<ggml_lbfgs_iteration_data *>(alloca(sizeof(struct ggml_lbfgs_iteration_data)*m));

    for (int i = 0; i < m; ++i) {
        lm[i].alpha = 0.0f;
        lm[i].ys    = 0.0f;
        lm[i].s     = static_cast<float *>(ggml_new_tensor_1d(ctx, GGML_TYPE_F32, nx)->data);
        lm[i].y     = static_cast<float *>(ggml_new_tensor_1d(ctx, GGML_TYPE_F32, nx)->data);
    }
    ...
    while (true) {
        ...
        if (ls < 0) {
            ...
            return static_cast<ggml_opt_result>(ls);
        }
